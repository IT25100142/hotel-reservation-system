...
<div class="container py-5 mt-4">
    <div class="card shadow p-4 mx-auto border-top border-danger border-4" style="max-width: 500px;">
        <div class="text-center mb-4">
            ...
            <h2 class="fw-bold mt-2 text-danger">Delete Invoice</h2>
            <p class="text-muted small">Remove an incorrect payment record.</p>
        </div>

        <% if ("true".equals(request.getParameter("success"))) { %>
        <div class="alert alert-success shadow-sm">Invoice successfully deleted!</div>
        <% } else if ("true".equals(request.getParameter("error"))) { %>
        <div class="alert alert-danger shadow-sm">Invoice ID not found.</div>
        <% } %>

        <form action="DeletePaymentServlet" method="POST">
            <div class="mb-4">
                <label class="form-label fw-bold">Invoice ID to Delete</label>
                <input type="text" name="invoiceId" class="form-control border-danger" placeholder="INV-..." required>
            </div>
            <button type="submit" class="btn btn-danger w-100 fw-bold shadow-sm" onclick="return confirm('Delete this invoice permanently?');">Delete Invoice</button>
        </form>
    </div>
</div>
</body>
