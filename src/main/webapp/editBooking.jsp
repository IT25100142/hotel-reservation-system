...
<div class="container py-5 mt-4">
    <div class="card shadow p-4 mx-auto border-top border-info border-4" style="max-width: 500px;">
        <div class="text-center mb-4">
            <i class="bi bi-calendar-range text-info" style="font-size: 3rem;"></i>
            <h2 class="fw-bold mt-2">Modify Reservation</h2>
        </div>

        <% if ("true".equals(request.getParameter("success"))) { %>
        <div class="alert alert-success shadow-sm"><i class="bi bi-check-circle me-2"></i>Booking successfully updated!</div>
        <% } else if ("true".equals(request.getParameter("error"))) { %>
        <div class="alert alert-danger shadow-sm"><i class="bi bi-x-circle me-2"></i>Booking ID not found.</div>
        <% } %>

        <form action="UpdateReservationServlet" method="POST">
            <div class="mb-3">
                <label class="form-label fw-bold">Booking ID</label>
                <input type="text" name="bookingId" class="form-control" placeholder="BKG-..." required>
            </div>
            <div class="mb-4">
                <label class="form-label fw-bold">Extend Check-Out Date</label>
                <input type="date" name="checkOutDate" class="form-control" required>
            </div>
            <button type="submit" class="btn btn-info text-white w-100 fw-bold shadow-sm">Update Booking</button>
        </form>
    </div>
</div>
</body>
